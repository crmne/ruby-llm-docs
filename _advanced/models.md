...
If your gem directory is read-only, configure a writable location with `config.model_registry_file` (v1.9.0+). See the [Configuration Guide]({% link _introduction/configuration.md %}#model-registry-file) for details.
...
*   See [Configuration Guide]({% link _introduction/configuration.md %}).
