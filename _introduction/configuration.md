...
Each provider has its own key (and sometimes region or project settings). For the full list of providers, organization headers, Vertex AI authentication, and OpenAI-compatible custom endpoints, see [Provider Setup and Custom Endpoints]({% link _introduction/configuration-providers.md %}).
...
- [Provider Setup and Custom Endpoints]({% link _introduction/configuration-providers.md %}) - every provider's keys, OpenAI organization headers, Vertex AI auth, and OpenAI-compatible endpoints.
- [Connection, Logging and Contexts]({% link _introduction/configuration-connection.md %}) - timeouts, retries, proxies, debug logging, the model registry file, and isolated per-tenant contexts.
- [Configuration Reference]({% link _introduction/configuration-reference.md %}) - the complete option list in one block.
